...
#package.harness <- function(path='.', out.file='aaa.R')
#{
#  source.files <- list.files(path, '\\.R$')
#  fn.finder <- function(a.file)
#  {
#    lines <- readLines(a.file)
#    gs <- lines[grep('^\\s*guard\\s*\\(', lines, perl=TRUE)]
#    ns <- sub('^\\s*guard\\s*\\((\\.?[^.]+).*$', '\\1', gs, perl=TRUE)
#    ns <- sub(',.*$','', ns, perl=TRUE)
#    unique(ns)
#  }
#  fn.harness <- function(a.fn)
#  {
#    function.fmt <- "%s <- function(...) UseFunction('%s',...)"
#    register.fmt <- "register('%s', environment())"
#    c(sprintf(function.fmt,a.fn,a.fn), sprintf(register.fmt,a.fn), "")
#  }
#
#  abstract.fns <- sapply(array(source.files), fn.finder)
#  abstract.fns <- do.call(c, abstract.fns)
#  out.source <- sapply(array(abstract.fns), fn.harness)
#
#  header <- sprintf("# Auto-generated by futile.paradigm on %s", Sys.time())
#
#  # Use this construction to avoid assumptions about path separators
#  old.path <- getwd()
#  setwd(path)
#  if (file.exists(out.file)) stop(sprintf("Output '%s' already exists",out.file))
#  writeLines(c(header,out.source), out.file)
#  setwd(old.path)
#  invisible()
#}
